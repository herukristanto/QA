...
include "koneksi.php";
 ...
 if(!empty($unit)){
   // if(!empty($bulan)){
   //   if(!empty($tahun)){
   // $query   = "SELECT DISTINCT kode_indikator, Kategori, Numerator, Denominator FROM V_Indikator_Kejadian WHERE kode_u= '$unit' ORDER BY kode_indikator ASC";
   $query   = "SELECT Kode, Numerator, Denominator, Kategori FROM M_Indikator WHERE Unit= '$unit' AND Status= 'X' ORDER BY Kode ASC";
   ...
   $no=1;
     ...
     <table class='table table-striped table-bordered table-hover'>
       ...
       while($row     = sqlsrv_fetch_array($data)){
         ...
         // $query1      = "SELECT COUNT(Kode) as jml FROM T_Kejadian_a WHERE kode_indikator= '$kodeindi'";
