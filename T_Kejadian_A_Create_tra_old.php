...
{
  #ambil data insiden
  $query      = "SELECT * from V_Insiden WHERE Status_Insiden = 'X'";
  $sql        = sqlsrv_query($conn, $query);
  $arrinsiden = array();
  while ($row = sqlsrv_fetch_array($sql)){
    array_push($arrinsiden, $row); 
    }
  echo json_encode($arrinsiden);
  exit;
  }

{
  #ambil data tipe
  if(isset($_GET['action']) && $_GET['action'] == "getTipe"){
...
{
  #ambil data sub tipe
  $query      = "SELECT * FROM V_Insiden WHERE Tipe_insiden AND Status_Tipe ='X'";
  $sql        = sqlsrv_query($conn, $query);
  $arrsubtipe = array();
  while ($row = sqlsrv_fetch_array($sql)) {
    arra_push($arrsubtipe, $row);
    }
  echo json_encode($arrsubtipe);
  exit;
  }
// {
