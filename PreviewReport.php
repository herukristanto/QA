			...
			for($x=1;$x<=$arrlengthsblm;$x++)
 			 {	$sttlsblm = 0;
				...
				$standarsblm = $arrStandarsblm[$x];
			 	$quesblm = "SELECT count (*) as jml FROM V_RepDetail WHERE Departemen = '".$dept."' AND Deskripsi = '".$unitkerja."' AND DeskAspek = '".$aspeksblm."'
				AND MONTH(Tgl) = '".$blnsblm."' AND YEAR(Tgl) = '".$tahunsblm."' AND Status = 'X' ";
			...
			for($x=1;$x<=$arrlength;$x++)
 			 {	$sttl = 0;
				...
				$standar = $arrStandar[$x];
			 	$que = "SELECT count (*) as jml FROM V_RepDetail WHERE Departemen = '".$dept."' AND Deskripsi = '".$unitkerja."' AND DeskAspek = '".$aspek."'
				AND MONTH(Tgl) = '".$bln."' AND YEAR(Tgl) = '".$tahun."' AND Status = 'X' ";
