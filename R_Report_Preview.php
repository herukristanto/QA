...
$querysblm	= "SELECT * FROM V_KatUnKer WHERE kode_dept = '".$dept."' AND Unit_Kerja = '".$unitkerja."' ORDER BY Kode ASC";	
	
/*$querysblm	= "SELECT * FROM V_Report WHERE Departemen = '".$dept."' AND Unit = '".$unitkerja."' ORDER BY Kode ASC";*/	
			
/*$querysblm	= "SELECT * FROM V_Report WHERE Departemen = '".$dept."' AND Unit = '".$unitkerja."' AND YEAR(Tgl_Kejadian) = '".$tahunsblm."'
			ORDER BY Kode ASC";*/				

//$querysblm	= "SELECT * FROM V_GroupIndi WHERE kode_dept = '".$dept."' AND Unit_Kerja = '".$unitkerja."' AND Status = 'X' ORDER BY kode_dept ASC";				
	
$sqlsblm	= sqlsrv_query($conn,$querysblm);

if  ($sqlsblm){
echo "
		<table hidden id=\"TableDetail\" border=\"1\" align=\"center\" cellspacing=\"0\" cellpadding=\"0\">
		...
		<td align=\"center\" width=\"80\"><b>Kode</b></td>
			...
			for($x=1;$x<=$arrlengthsblm;$x++)
 			 {	$sttlsblm = 0;
				...
				$standarsblm 	= $arrStandarsblm[$x];
			 	...
			 	$quesblm 		= "SELECT count (*) as jml FROM V_RepDetail WHERE Departemen = '".$dept."' AND Unit = '".$unitkerja."'
				AND Aspek like '".$aspeksblm."' AND MONTH(Tgl_Kejadian) = '".$blnsblm."' AND YEAR(Tgl_Kejadian) = '".$tahunsblm."' AND Status = 'X'";
				
				/*$quesblm 		= "SELECT count (*) as jml FROM V_RepDetail WHERE Departemen = '".$dept."' AND Unit = '".$unitkerja."'
				AND Aspek = '".$aspeksblm."' AND MONTH(Tgl_Kejadian) = '".$blnsblm."' AND YEAR(Tgl_Kejadian) = '".$tahunsblm."' AND Status = 'X'";*/
...
$query	= "SELECT * FROM V_KatUnKer WHERE kode_dept = '".$dept."' AND Unit_Kerja = '".$unitkerja."' ORDER BY Kode ASC";
		...
		while($rs = sqlsrv_fetch_array($sql)){
			...
			for($x=1;$x<=$arrlength;$x++)
 			 {	$sttl = 0;
				...
				$que	= "SELECT count (*) as jml FROM V_RepDetail WHERE Departemen = '".$dept."' AND Unit = '".$unitkerja."'
				AND Aspek like '".$aspek."' AND MONTH(Tgl_Kejadian) = '".$bln."' AND YEAR(Tgl_Kejadian) = '".$tahun."' AND Status = 'X'";
